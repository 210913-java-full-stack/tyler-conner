...
###########################################################
DROP DATABASE IF EXISTS tc_assignment;
CREATE DATABASE tc_assignment;
...
SELECT SUM(a.balance) AS "Total Deposits"
FROM accounts a 
...
WHERE ac.junction_id = 1;

UPDATE accounts a 
JOIN accounts_customers ac ON a.account_id =ac.account_id 
SET a.balance = a.balance + 199.99
WHERE ac.junction_id = 3;
# Change Amanda Smith's last name to "Lastname"

UPDATE customers c 
SET c.name = "Amanada Lastname" 
